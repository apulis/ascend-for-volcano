...
func buildNpuResourceList(cpu string, memory string, npu string) v1.ResourceList {
	...
	return v1.ResourceList{
		v1.ResourceCPU:    resource.MustParse(cpu),
		v1.ResourceMemory: resource.MustParse(memory),
		npuResourceName:   resource.MustParse(npu),
	}
}

func addNodeNpuTop(Annotations map[string]string, top string) {
	Annotations[npuResourceName] = top
}

// BuildNode builts npu node object
func buildNpuNode(nodeName string, cpu string, mem string, npuAllocatecNum string, npuTop string) *api.NodeInfo {
	nodeCapacity := buildNpuResourceList(cpu, mem, strconv.Itoa(npuNumbPerNode))
	nodeAlloc := buildNpuResourceList(cpu, mem, npuAllocatecNum)
	...
	if npuAllocatecNum != "0" {
		addNodeNpuTop(v1node.Annotations, npuTop)
	}
...
func buildNpuPod(namespace, groupName string, podName string, nodeName string,
	reqCpuNum string, reqMem string, npuNeedNum string) *v1.Pod {

	pod := util.BuildPod(namespace, podName, nodeName, v1.PodPending,
		buildNpuResourceList(reqCpuNum, reqMem, npuNeedNum), groupName, make(map[string]string), make(map[string]string))
...
func buildValidJobTests() []TestCaseStruct {
	...
	podOneCard := buildNpuPod(namespaceName, podGroupName, "p0", nodeName1, "10", "1Gi", "1")
	podTwoCard := buildNpuPod(namespaceName, podGroupName, "p0", nodeName1, "10", "1Gi", "2")
	podEightCard1 := buildNpuPod(namespaceName, podGroupName, "p1", nodeName1, "1", "1Gi", "8")
	podEightCard2 := buildNpuPod(namespaceName, podGroupName, "p2", nodeName2, "1", "1Gi", "8")
	podErrorCard := buildNpuPod(namespaceName, podGroupName, "p2", nodeName1, "1", "1Gi", "3")
	podCpuCard := buildNpuPod(namespaceName, podGroupName, "p3", nodeName1, "1", "1Gi", "0")
	podErrorDistributeCard := buildNpuPod(namespaceName, podGroupName, "16", nodeName1, "1", "1Gi", "16")
	podDistributeCard1 := buildNpuPod(namespaceName, podGroupName, "pdis1", nodeName1, "1", "1Gi", "4")
	podDistributeCard2 := buildNpuPod(namespaceName, podGroupName, "pdis2", nodeName2, "1", "1Gi", "4")

	nodeStand1 := buildNpuNode(nodeName1, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6,Ascend910-7")
	nodeStand2 := buildNpuNode(nodeName2, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6,Ascend910-7")
	nodeFault1 := buildNpuNode(nodeName2, "192", "755Gi", "7",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-5,Ascend910-6,Ascend910-7")
	nodeStand3 := buildNpuNode(nodeName1, "192", "755Gi", "8",
		"Ascend910-0")
	...
	tests = append(tests, initTestCase("distribute job success test", []*schedulingv1.PodGroup{podGroup},
		[]*schedulingv1.Queue{queue1}, []*v1.Pod{podEightCard1, podEightCard2}, []*api.NodeInfo{nodeStand1, nodeStand2}, true))
...
func buildNpuPredicateTests() []TestCaseStruct {
	...
	npuPod := buildNpuPod(namespaceName, podGroupName, "p0", nodeName1, "10", "1Gi", "8")
	cpuPod := buildNpuPod(namespaceName, podGroupName, "p3", nodeName1, "1", "1Gi", "0")

	npuNodeStand1 := buildNpuNode(nodeName1, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6,Ascend910-7")
	stableNode := buildNpuNode(nodeName2, "192", "755Gi", "8",
		"Ascend910-0,Ascend910-1,Ascend910-2,Ascend910-3,Ascend910-4,Ascend910-5,Ascend910-6")
	cpuNodeStand1 := buildNpuNode(nodeName1, "192", "755Gi", "0", "")
...
func buildAnnotationRandom(leftHccsNum int, rightHccsNum int) string {
	...
	generateRandCard := func() {
		randCard1 = rand.Int() % 4
		for {
			if randCard2 = (rand.Int() + 1) % 4; randCard1 != randCard2 {
				break
			}
		}
	}
...
func addNpuNodePool(nodeName string, npuAllocatecNum int, nodePool map[string]*api.NodeInfo) {
	...
	for leftHccsNum := 0; leftHccsNum <= npuNumPerHccs; leftHccsNum++ {
		for rightHccsNum := 0; rightHccsNum <= npuNumPerHccs; rightHccsNum++ {
			...
			p := buildNpuNode(nodeName, "96", "64Gi", strconv.Itoa(npuAllocatecNum), annotation)
			nodePool[key] = p
		}
	}
}
...
func buildNpuNodePool(nodeAlloList []NodeAllocate) map[string]*api.NodeInfo {
	nodePool := make(map[string]*api.NodeInfo)
...
func buildNpuPodPool(namespaceName string, podGroupName string, nodeAlloList []NodeAllocate) map[string]*v1.Pod {
	...
	podPool := make(map[string]*v1.Pod)
	for podNo := 0; podNo <= npuNumbPerNode; podNo++ {
		for _, nodAlloc := range nodeAlloList {
			podName := PodNamePrefix + strconv.Itoa(podNo) + "_" + nodAlloc.nodeName
			p := buildNpuPod(namespaceName, podGroupName, podName, nodAlloc.nodeName,
...
func getTaskInfo(podName string) (string, string, error) {
	...
	if _, err := strconv.Atoi(needCardnum); err != nil {
		return "", "", errors.New("input podName Format error: needCardNum is not 'int' type  after 'p' and before '_' ")
	}
...
func testNodeByJobBatchNodeOrderFn(ssn *framework.Session, nodes []*api.NodeInfo, test *TestCaseStruct, testNo int) error {
	var expType map[string]map[string]float64
	var ok bool
	...
	for _, jobInfo := range ssn.Jobs {
		for _, task := range jobInfo.Tasks {
			...
			if expType, ok = test.expected.(map[string]map[string]float64); !ok {
				return errors.New("test case expect type is error")
			}
			if expectScore := expType[taskID][task.NodeName]; expectScore != score[task.NodeName] {
				fmt.Println(` "case" + strconv.Itoa(testNo) + " : " + test.name + " task:" + 
				taskID + " on node:" + task.NodeName + " test failed, 
				expect have score:" + strconv.Itoa(int(expectScore)) + ", 
				but get:" + strconv.Itoa(int(score[task.NodeName]))`)
			...
			defer npuDeallocateFunc(&framework.Event{
				Task: task,
			}, ssn.Nodes)
		}
	}
