...
func getBestNodeAndTop(needCards int, priNodeGroups []map[string]*npuPriNodeInf) (string, *hccsInf, error) {
	...
	klog.V(logInfoLev).Infof("%s getBestNodeAndTop selectPriGroup:%v capacity:%d", PluginName, selectPriGroup, capacity)
...
func checkNodeNpuStabilize(nodeNpuIdleNumFromTop int, nodeNpuIdleNumFromIdle int) error {
	if nodeNpuIdleNumFromTop != nodeNpuIdleNumFromIdle {
		return fmt.Errorf("node annotations(%d) not same node idle(%d)", nodeNpuIdleNumFromIdle, nodeNpuIdleNumFromTop)
	}
...
func getTopToIntArray(topStr string) []int {
	...
	for _, cardStr = range topStrArray {
		...
		if err != nil {
			klog.V(logErrorLev).Infof("%s getTopFromNode conv faild %v", PluginName, err)
			return nil
		}
...
func getTopFromNode(node *api.NodeInfo) []int {
	...
	if !ok {
		klog.V(logErrorLev).Infof("%s getTopFromNode top nil:%v", PluginName, node.Node.Annotations[node.Name])
		return nil
	}
	...
	klog.V(logDebugLev).Infof("%s getTopFromNode %v", PluginName, topInt)
	return topInt
}
...
func checkNodeNpuByTask(task *api.TaskInfo, node *api.NodeInfo) error {
	...
	klog.V(logInfoLev).Infof("%s checkNodeNpuByTask top:%v", PluginName, nodeNpuTopology)
...
func reloadNewTopToNode(node *api.NodeInfo, newNodeTopStr string) error {

	node.Node.Annotations[npuResourceName] = newNodeTopStr
...
func useAnnotation(node *api.NodeInfo, task *api.TaskInfo) {
	...
	nodeTopInt := getTopIntFromAnnotations(node.Node.Annotations)
	if nodeTopInt == nil {
		klog.V(logErrorLev).Infof("%s useAnnotation node(%s) top nil", PluginName, node.Name)
		return
	}
	// delete the use top
	klog.V(logDebugLev).Infof("%s getRealTopAfterAlloc top nod：%v , task: %v", PluginName, nodeTopInt, taskTopInt)
	...
	err := reloadNewTopToNode(node, newNodeTopStr)
	if err != nil {
